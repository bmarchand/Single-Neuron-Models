...
def subMembPot(state,string): #membrane potential before NL.
	...
	#Nbbp = state.basisBackProp.shape[0]

	for g in range(state.Ng):

		Basis = state.basisKer

		if state.flag[g]=='backprop':

			if string=='training':

				inp = state.input[Nneur[g][0]:(Nneur[g][-1]-1)]
				inpbp = state.input[Nneur[g][-1]-1]

			elif string=='test':

				inp = state.input_test[Nneur[g][0]:(Nneur[g][-1]-1)]
				inpbp = state.input_test[Nneur[g][-1]-1]

			X = cv(inp,Basis,state)[:,:Nsteps] #+1 in there.
			Xbp = cv(inpbp,state.basisBackProp,state)[:,:Nsteps]
	
			ming = Nneur[g][0]
			maxg = Nneur[g][-1]-1

			u1 = np.dot(state.paramKer[ming*Nb:maxg*Nb],X)
			u2 = np.dot(state.paramKer[maxg*Nb:(maxg*Nb+Nbbp)],Xbp)
