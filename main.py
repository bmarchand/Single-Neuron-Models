...
class SpikingMechanism: #gather parameters for spiking.
	...
	threshold = 40. #bio-inspired threshold value.
	PSP_size = 25. # mV: Roughly std of the memb. pot. in a compartment before NL 	  
	ASP_size = 0. # [mV] size of After-Spike-Potential (ASP)
...
class RunParameters:
	...
	total_time = 1000000. #total simulation time
...
class FitParameters: # FitParameters is one component of the TwoLayerModel class
	...
	knots = range(-30,40,15) #knots for NL (unused)
	...
	basisNL = fun.NaturalSpline(knots,bnds,100000.) #basis for NL (splines)
	basisNLder = fun.DerNaturalSpline(knots,bnds,100000.) #derivative of above.
	basisNLSecDer = fun.SecDerNaturalSpline(knots,bnds,100000.) #second derivative.
